...
class fk_dashboard extends oxAdminView
{
  ...
  protected $_statstart = "2010-04-10"; // TODO: get first sale, use that date
  ...
  {
      require_once("../modules/fk_dashboard/inc/google-chart.inc.php");
      ...
      {                               
        ...
        if( $row != false && $row->recordCount() > 0 ) {
          // only one result row!
        	//while ( !$rows->EOF ) {                
            $orders = $row->fields["orders"];            
            $totalsum = $row->fields["totalsum"];
            $avgsum = $row->fields["avgsum"];   
            $vouchersvalue = $row->fields["vouchersvalue"];
            $delcostpaid = $row->fields["delcostpaid"];    		              
